	...
	if (mem_avail) {
		...
		for (int j = 0; index < num_pages; j++){
			if (_mem_stat[j].proc == 0){
				...
				_mem_stat[j].index = index;
				index++;
				if (prev >= 0) _mem_stat[prev].next = j;
				if (index == num_pages) _mem_stat[j].next = -1;
				prev = j;
				
				// Proc segment table allocation
				addr_t first_lv = get_first_lv(vAddr);
				struct page_table_t * page_table = NULL;
				page_table = get_page_table(first_lv, proc->seg_table);
				int sSize = -1;
				if (page_table == NULL) {
					// Create new segment with corresponding virtual index
					sSize = (proc->seg_table->size += 1) - 1;
